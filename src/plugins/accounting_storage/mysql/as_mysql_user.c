...
static int _foreach_add_user(void *x, void *arg)
{
	...
	query = xstrdup_printf(
		"select deleted from %s where name='%s';",
		user_table, object->name);
	result = mysql_db_query_ret(add_user_cond->mysql_conn, query, 0);
	xfree(query);
	if (result) {
		MYSQL_ROW chk = mysql_fetch_row(result);
		if (chk && chk[0] &&
		    (atoi(chk[0]) == SLURMDB_DELETED_DEACTIVATED)) {
			mysql_free_result(result);
			error("User %s exists in deactivated state; activate first",
			      object->name);
			add_user_cond->rc = ESLURM_DB_RECORD_FROZEN;
			      ...
			      List user_list)
{
	...
	while ((object = list_next(itr))) {
		...
		{
			MYSQL_RES *fr = NULL;
			char *fq = xstrdup_printf(
				"select deleted from %s where name='%s';",
				user_table, object->name);
			fr = mysql_db_query_ret(mysql_conn, fq, 0);
			xfree(fq);
			if (fr) {
				MYSQL_ROW chk = mysql_fetch_row(fr);
				if (chk && chk[0] &&
				    (atoi(chk[0]) == SLURMDB_DELETED_DEACTIVATED)) {
				  ...
				  slurmdb_user_cond_t *user_cond)
{
	...
	if (!is_user_min_admin_level(mysql_conn, uid, SLURMDB_ADMIN_OPERATOR)) {
		if (slurmdbd_conf->flags & DBD_CONF_FLAG_DISABLE_COORD_DBD) {
			error("Coordinator privilege revoked with DisableCoordDBD, only admins/operators can deactivate users.");
			errno = ESLURM_ACCESS_DENIED;
			return NULL;
		}

		/*
		 * Allow coordinators to deactivate users from accounts that
		 * they coordinate. After we have gotten every association that
		 * the users belong to, check that the coordinator has access
		 * to modify every affected account.
		 */
		is_coord = is_user_any_coord(mysql_conn, &user);
		if (!is_coord) {
			error("Only admins/coordinators can deactivate users");
			errno = ESLURM_ACCESS_DENIED;
			return NULL;
		}
	}
	...
	/* Only handle this if we need to actually query the
	   user_table.  If a request comes in stating they want to
	   deactivate all users with default account of whatever then that
	   doesn't deal with the user_table.
	*/
	query = xstrdup_printf("select name from %s where deleted=0 %s;",
			       user_table, extra);
...
no_user_table:
	...
	*/
	...
	while ((object = list_next(itr))) {
		...
		/*
		 * Skip empty names or else will select account associations
		 * and deactivate all associations.
		 */
		if (!object[0]) {
			list_delete_item(itr);
			continue;
		...
		user_rec->name = xstrdup(object);
		if (addto_update_list(mysql_conn->update_list,
				      SLURMDB_DEACTIVATE_USER, user_rec)
		    != SLURM_SUCCESS)
			slurmdb_destroy_user_rec(user_rec);
	}
	list_iterator_destroy(itr);
	...
	/* We need to deactivate these users from the wckey table */
	memset(&wckey_cond, 0, sizeof(slurmdb_wckey_cond_t));
	wckey_cond.user_list = assoc_cond.user_list;
	coord_list = as_mysql_deactivate_wckeys(mysql_conn, uid, &wckey_cond);
	FREE_NULL_LIST(coord_list);
	...
	xfree(query);
	if (rc != SLURM_SUCCESS) {
		error("Couldn't deactivate user coordinators");
		FREE_NULL_LIST(ret_list);
		return NULL;
	}
				  ...
				  slurmdb_user_cond_t *user_cond)
{
	...
	if (!(is_admin = is_user_min_admin_level(
		...
		if (slurmdbd_conf->flags & DBD_CONF_FLAG_DISABLE_COORD_DBD) {
			error("Coordinator privilege revoked with DisableCoordDBD, only admins/operators can deactivate coordinators.");
			errno = ESLURM_ACCESS_DENIED;
			return NULL;
		}
		if (!is_user_any_coord(mysql_conn, &user)) {
			error("Only admins/coordinators can "
			      "deactivate coordinators");
			errno = ESLURM_ACCESS_DENIED;
			return NULL;
		}
	}
...
#endif
		if (addto_update_list(mysql_conn->update_list,
				      SLURMDB_DEACTIVATE_COORD, user_rec)
		    != SLURM_SUCCESS)
			slurmdb_destroy_user_rec(user_rec);
	}
	list_iterator_destroy(itr);
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
		xstrcat(extra, "where (deleted=0 || deleted=1 || deleted=%d)", SLURMDB_USER_DEACTIVATED);
	else if (user_cond->only_deactivated)
		xstrcat(extra, "where deleted=%d", SLURMDB_USER_DEACTIVATED);
#else
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
		user_cond->assoc_cond->only_deactivated = user_cond->only_deactivated;
#endif
