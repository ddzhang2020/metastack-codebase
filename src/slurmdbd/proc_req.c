...
static int _activate_accounts_cond(slurmdbd_conn_t *slurmdbd_conn,
			      persist_msg_t *msg,
			      buf_t **out_buffer)
{
	int rc = SLURM_SUCCESS;
	...
	debug2("DBD_ACTIVATE_ACCOUNTS_COND: called in CONN %d",
	       slurmdbd_conn->conn->fd);
	...
	if (!(comment = acct_storage_g_activate_accounts_cond(
		      slurmdbd_conn->db_conn,
		      slurmdbd_conn->conn->auth_uid,
		      modify_msg->cond,
		      modify_msg->rec))) {
	...
	*out_buffer = slurm_persist_make_rc_msg(slurmdbd_conn->conn,
						rc, comment,
						DBD_ACTIVATE_ACCOUNTS_COND);
	if (free_comment)
		xfree(comment);
	return rc;
}

static int _activate_users_cond(slurmdbd_conn_t *slurmdbd_conn,
			   persist_msg_t *msg,
			   buf_t **out_buffer)
{
	int rc = SLURM_SUCCESS;
	...
	debug2("DBD_ACTIVATE_USERS_COND: called in CONN %d",
	       slurmdbd_conn->conn->fd);
	...
	if (!(comment = acct_storage_g_activate_users_cond(
		      slurmdbd_conn->db_conn,
		      slurmdbd_conn->conn->auth_uid,
		      modify_msg->cond,
		      modify_msg->rec))) {
...
static int _deactivate_accounts(slurmdbd_conn_t *slurmdbd_conn, persist_msg_t *msg,
			    buf_t **out_buffer)
{
	int rc = SLURM_SUCCESS;
	dbd_cond_msg_t *get_msg = msg->data;
	...
	debug2("DBD_DEACTIVATE_ACCOUNTS: called in CONN %d",
	       slurmdbd_conn->conn->fd);

	if (!(list_msg.my_list = acct_storage_g_deactivate_accounts(
		      slurmdbd_conn->db_conn, slurmdbd_conn->conn->auth_uid,
		      get_msg->cond))) {
		if (errno == ESLURM_ACCESS_DENIED) {
			comment = "Your user doesn't have privilege to perform this action";
		...
		error("CONN:%d %s", slurmdbd_conn->conn->fd, comment);
		*out_buffer = slurm_persist_make_rc_msg(
			slurmdbd_conn->conn, rc, comment, DBD_DEACTIVATE_ACCOUNTS);
		return rc;
	}
	list_msg.return_code = errno;
...
static int _deactivate_users(slurmdbd_conn_t *slurmdbd_conn, persist_msg_t *msg,
			 buf_t **out_buffer)
{
	int rc = SLURM_SUCCESS;
	dbd_cond_msg_t *get_msg = msg->data;
	...
	debug2("DBD_DEACTIVATE_USERS: called in CONN %d", slurmdbd_conn->conn->fd);

	if (!(list_msg.my_list = acct_storage_g_deactivate_users(
		      slurmdbd_conn->db_conn, slurmdbd_conn->conn->auth_uid,
		      get_msg->cond))) {
		if (errno == ESLURM_ACCESS_DENIED) {
			comment = "Your user doesn't have privilege to perform this action";
		...
		error("CONN:%d %s", slurmdbd_conn->conn->fd, comment);
		*out_buffer = slurm_persist_make_rc_msg(
			slurmdbd_conn->conn, rc, comment, DBD_DEACTIVATE_USERS);
		return rc;
	}
	list_msg.return_code = errno;
...
static int _deactivate_assocs(slurmdbd_conn_t *slurmdbd_conn, persist_msg_t *msg,
			  buf_t **out_buffer)
{
	int rc = SLURM_SUCCESS;
	dbd_cond_msg_t *get_msg = msg->data;
	...
	debug2("DBD_DEACTIVATE_ASSOCS: called in CONN %d", slurmdbd_conn->conn->fd);
	...
	if (!(list_msg.my_list = acct_storage_g_deactivate_assocs(
		      slurmdbd_conn->db_conn, slurmdbd_conn->conn->auth_uid,
		      get_msg->cond))) {
		if (errno == ESLURM_ACCESS_DENIED) {
			comment = "Your user doesn't have privilege to perform this action";
		...
		error("CONN:%d %s", slurmdbd_conn->conn->fd, comment);
		*out_buffer = slurm_persist_make_rc_msg(
			slurmdbd_conn->conn, rc, comment, DBD_DEACTIVATE_ASSOCS);
		return rc;
	}
	list_msg.return_code = errno;
...
extern int proc_req(void *conn, persist_msg_t *msg, buf_t **out_buffer)
{
	...
	case DBD_REMOVE_USERS:
		...
		break;
#ifdef __METASTACK_OPT_USER_DEACTIVATE
	case DBD_ACTIVATE_ACCOUNTS_COND:
		rc = _activate_accounts_cond(slurmdbd_conn, msg, out_buffer);
		break;
	case DBD_ACTIVATE_USERS_COND:
		rc = _activate_users_cond(slurmdbd_conn, msg, out_buffer);
		break;
	case DBD_DEACTIVATE_ACCOUNTS:
		rc = _deactivate_accounts(slurmdbd_conn, msg, out_buffer);
		break;
	case DBD_DEACTIVATE_USERS:
		rc = _deactivate_users(slurmdbd_conn, msg, out_buffer);
		break;
	case DBD_DEACTIVATE_ASSOCS:
		rc = _deactivate_assocs(slurmdbd_conn, msg, out_buffer);
		break;
#endif
